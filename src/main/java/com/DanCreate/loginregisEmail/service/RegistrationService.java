...
public class RegistrationService {
    ...
    public String register(RegistrationDto requestDto) {
        ...
        Boolean isValidateEmail = emailValidatorService.test(requestDto.getEmail());
        if (!isValidateEmail) {
        ...
        emailService.send(
                requestDto.getEmail(),
                buildEmail(requestDto.getFirstname(), link));
        return token;
    }

    @Transactional
    public String confirmToken(String token) {
        TokenKey tokenKey = tokenKeyService
                .getToken(token)
                .orElseThrow(() ->
                        new UsernameNotFoundException("token not found"));

        if (tokenKey.getConfirmedAt() != null) {
            /*
        ...
        /*
            có thể dùng để trả về 1 dưới dạng 1 json, error 500
            throw new IllegalStateException("token expired");
            throw new IllegalStateException("email already confirmed");
           */
        LocalDateTime expiredAt = tokenKey.getExpiresAt();
